...
EOF
  ...
  for i in "${!CONFIGS[@]}"; do
    ...
    cat >> docker-compose.yml <<EOF
      ...
      DOMAIN_${DOMAIN_NUM}_MAC: \${DOMAIN_${DOMAIN_NUM}_MAC:-}
      DOMAIN_${DOMAIN_NUM}_CONFIG: \${DOMAIN_${DOMAIN_NUM}_CONFIG:-default}
      DOMAIN_${DOMAIN_NUM}_USE_HTTPS: \${DOMAIN_${DOMAIN_NUM}_USE_HTTPS:-no}
      DOMAIN_${DOMAIN_NUM}_USE_SSL: \${DOMAIN_${DOMAIN_NUM}_USE_SSL:-no}
      DOMAIN_${DOMAIN_NUM}_READ_TIMEOUT: \${DOMAIN_${DOMAIN_NUM}_READ_TIMEOUT:-10s}
      DOMAIN_${DOMAIN_NUM}_IDLE_SERVICE: \${DOMAIN_${DOMAIN_NUM}_IDLE_SERVICE:-}
EOF
  done
